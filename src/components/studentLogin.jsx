 ...
 useEffect(() => {
  ...
  const handleVerify = async(tokens) => {
    await axios.post("https://register-api-cloud.vercel.app/student/verify",{
      name:user,
      header:token
    }).then(res =>{setName(res.data);nav(`/student/${name}`);console.log(res.data); alert(user +""+ "is verified successfully")}).catch(err => {alert(user + "is not verified");console.log(err)})
